(() => {
  // Reuse the existing email signup speed bump, not server authentication.
  ...
  if (!window.COURSE_ACCESS) {
    ...
    location.replace('/free-course/?next=' + (query.get('view') === 'video' ? 'orientation-video' : 'orientation'));
    return;
  }
