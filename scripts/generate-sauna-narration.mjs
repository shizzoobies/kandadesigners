...
const root = 'public/training-samples/sauna/';
const envFile = process.env.KA_ENV_FILE || '.dev.vars';
const line = fs.readFileSync(envFile, 'utf8').split(/\r?\n/).find(value => value.startsWith('ELEVENLABS_API_KEY='));
if (!line) throw new Error('The local ElevenLabs key is missing.');
const apiKey = line.slice('ELEVENLABS_API_KEY='.length).trim().replace(/^["']|["']$/g, '');
const context = { window: {} };
vm.runInNewContext(fs.readFileSync(root + 'narration.js', 'utf8'), context);
const scripts = context.window.saunaNarration;
