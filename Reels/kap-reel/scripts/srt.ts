...
/**
 * The six shapes a tutorial delivers in: the showcase reels' five, which are
 * three crops of the 15 second cut and two of the 45 second one, plus the 15
 * second cut at 1920x1080. See TUTORIAL_SHAPES in scripts/deliver.ts for why
 * the tutorials carry that sixth and the showcase reels do not.
 *
...
/** Every caption target for one reel: five for a showcase reel, six for a tutorial. */
...
function main(argv: string[]): void {
  ...
  if (argv.includes("--check")) {
    if (problems.length === 0) {
      console.log(`srt: all ${targets.length} ${reelArg} tables validate.`);
    }
