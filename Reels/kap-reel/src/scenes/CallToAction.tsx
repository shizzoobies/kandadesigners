 ...
 * The wordmark's T 5.2 lands at relative 45.5, so its first glyph is on screen
 * on frame 46 and the contact block arrives one frame later, at 47: the last
 * thing the lockup does and the first thing the copy does are the same gesture.
 * Everything is on screen from 47; the last wordmark glyph finishes its ease at
 * 60 and nothing moves after that, so the card is frozen for the final 18
 * frames. That is under CTA_HOLD_MIN_FRAMES and the owner took the trade with
 * the number in front of them.
 *
 * 45 second cut. LINKEDIN_CALL_TO_ACTION is 124 frames and did not have to
 * change. The draw takes 84, the wordmark starts on frame 58, the copy arrives
 * at 59, and the finished card is frozen from frame 76 to the end, which is 48
 * frames and clears the minimum.
 *
 * Both copy cues moved again later on 2026-09-05, with DRAW_START_T below. The
 * wordmark starts at relative 45.5 rather than 41.7 in the 15 second cut and at
 * 57.9 rather than 53.0 in the 45 second one, which is more than the two frame
 * threshold at which the SRT tables have to be re-cut, so they were: the url
 * and the phone now begin at absolute 419 and 1285 in scripts/srt.ts.
 */
 ...
 * path that had drawn nothing. The QA harness failed check (e) on both card
 * frames of all twelve compositions: ink coverage 0.044 to 0.096 percent
 * against a flat 0.2 percent floor.
 *
 * The first answer to that, earlier the same day, was to start the card at
 * T 2.119, nine tenths of the way round the frame, which cleared the floor by
 * measurement and cost the thing the owner asked for. The mouse dragging the
 * browser frame into being is the opening gesture of the piece, and at 2.119 it
 * has already happened off screen: the card cut in on a nearly finished
 * rectangle.
 *
 * Owner decision 2026-09-05, later the same day: the draw comes back. The card
 * opens where the path is one sixth drawn, which is easeInOutCubic(p) = 0.16,
 * so p = 0.04^(1/3) = 0.34200, and the draw runs from T 0.35 to T 2.85, giving
 * T = 0.35 + 2.5 p = 1.205. mouseIn ramps over T 0 to 0.35, so the pointer is
 * fully in on frame 0 and it opens on a cursor with a short segment of line
 * behind it. The sweep up the left edge, along the top, the three window dots
 * popping and the drop down the right are all in the cut again.
 *
 * That frame is not a blank one, but it is under the 0.2 percent floor, and it
 * always was: the browser frame is a five unit stroke on a 1340 unit stage,
 * about 2.7 canvas pixels at this box width, and the whole of it fully drawn,
 * mouse included, covers only 0.28 percent of the frame. All the ink on this
 * card is in the letters and they do not start until T 3. The floor is not the
 * wrong number for a finished picture; it is the wrong test for two frames of a
 * line being drawn, and check (e) now says so per frame rather than being
 * loosened. See checkBlankFrame() in scripts/qa/checks.ts and the QA section of
 * the README.
 *
 * The cost is that the copy cue had to move again: see CTA_TIMING above. The
 * remaining 5.795 seconds compress into the same 66 and 84 frames, so the piece
 * runs at 2.6x rather than 2.2x and every gesture in it is a fifth quicker than
 * it was at T 2.119, which is the price of putting the sweep back inside a beat
 * whose length the owner has already fixed.
 */
