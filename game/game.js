...
const TOWERS=[
  {name:'Rock Thrower',cost:30,dmg:15,range:2.5,rate:1.2,pSpd:5,splash:0,age:0,clr:'#8B7355',icon:'🪨'},
  {name:'Spike Trap',cost:20,dmg:10,range:1.5,rate:0.5,pSpd:8,splash:0,age:0,clr:'#556B2F',icon:'🌿'},
  {name:'Archer',cost:60,dmg:22,range:3.5,rate:0.8,pSpd:8,splash:0,age:1,clr:'#CD853F',icon:'🏹'},
  {name:'Spear Thrower',cost:80,dmg:40,range:2.8,rate:1.4,pSpd:6,splash:0,age:1,clr:'#B8860B',icon:'🗡️'},
  {name:'Crossbow',cost:120,dmg:45,range:3.8,rate:0.65,pSpd:10,splash:0,age:2,clr:'#808080',icon:'🎯'},
  {name:'Catapult',cost:160,dmg:65,range:3.5,rate:2.0,pSpd:4,splash:1.5,age:2,clr:'#8B4513',icon:'💥'},
  {name:'Cannon',cost:250,dmg:100,range:3.2,rate:2.2,pSpd:6,splash:1.8,age:3,clr:'#4A4A4A',icon:'💣'},
  {name:'Gatling',cost:300,dmg:18,range:3.0,rate:0.15,pSpd:12,splash:0,age:3,clr:'#556B2F',icon:'🔫'},
  {name:'Missile',cost:500,dmg:200,range:5.0,rate:2.8,pSpd:7,splash:2.0,age:4,clr:'#2F4F4F',icon:'🚀'},
  {name:'Sniper',cost:600,dmg:400,range:6.0,rate:3.0,pSpd:25,splash:0,age:4,clr:'#1C1C1C',icon:'🎯'},
  {name:'Laser',cost:1000,dmg:100,range:4.5,rate:0,pSpd:0,splash:0,age:5,clr:'#00CED1',icon:'⚡',isLaser:true},
  {name:'Plasma',cost:1500,dmg:500,range:4.0,rate:2.0,pSpd:8,splash:2.5,age:5,clr:'#9400D3',icon:'🔮'}
];

const ECONS=[
  {name:'Hut',cost:50,prod:0.5,age:0,clr:'#8B7355',icon:'🛖'},
  {name:'Farm',cost:120,prod:1.5,age:1,clr:'#DAA520',icon:'🌾'},
  {name:'Market',cost:300,prod:4,age:2,clr:'#B22222',icon:'🏪'},
  {name:'Factory',cost:700,prod:10,age:3,clr:'#708090',icon:'🏭'},
  {name:'Corp',cost:1500,prod:25,age:4,clr:'#4169E1',icon:'🏢'},
  {name:'Quantum Forge',cost:3500,prod:65,age:5,clr:'#00FFFF',icon:'⚛️'}
];
...
function spawnEnemy(){
  ...
  let colors=[['#c4a46c','#8B7355'],['#cd7f32','#B8860B'],['#808080','#B22222'],['#556B2F','#4A4A4A'],['#2F4F4F','#1C1C1C'],['#00CED1','#9400D3']];
  let ac=colors[Math.min(game.age,5)];
  game.enemies.push({
    hp,maxHp:hp,speed:spd,pathIdx:0,
    x:0,y:0,boss,reward:killReward(hp)*(boss?3:1),
    color:boss?ac[1]:ac[0],radius:boss?cellSize*0.4:cellSize*0.3,
    slow:0,slowTimer:0
  });
  game.waveSpawned++;
}
...
function spawnKillFx(e){
  for(let i=0;i<8;i++)game.particles.push({x:e.x,y:e.y,vx:(Math.random()-0.5)*150,vy:(Math.random()-0.5)*150,life:0.5,color:e.color,size:4});
...
function render(){
  ...
  // Grid
  for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){
    let x=offX+c*cellSize,y=offY+r*cellSize;
    let onPath=pathSet.has(c+','+r);
    ctx.fillStyle=onPath?age.path:age.color;
    ctx.fillRect(x,y,cellSize,cellSize);
    ctx.strokeStyle='rgba(0,0,0,0.15)';ctx.strokeRect(x,y,cellSize,cellSize);
  }
  ...
  for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){
    ...
    let pos=g2p(c,r),sz=cellSize*0.35;
    if(b.cat==='tower'){
      let def=TOWERS[b.typeId];
      ctx.save();ctx.translate(pos.x,pos.y);ctx.rotate(b.angle||0);
      ctx.fillStyle=def.clr;
      ctx.beginPath();ctx.moveTo(sz,0);ctx.lineTo(-sz,-sz*0.7);ctx.lineTo(-sz,sz*0.7);ctx.closePath();ctx.fill();
      ctx.restore();
      // Level dots
      for(let i=0;i<b.level;i++){
        ctx.fillStyle='#FFD700';ctx.beginPath();
        ctx.arc(pos.x-sz+i*6,pos.y-sz-4,2,0,Math.PI*2);ctx.fill();
      }
    } else {
      let def=ECONS[b.typeId];
      ctx.fillStyle=def.clr;
      ctx.beginPath();ctx.arc(pos.x,pos.y,sz,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='rgba(255,255,255,0.3)';
      ctx.beginPath();ctx.arc(pos.x,pos.y,sz*0.5,0,Math.PI*2);ctx.fill();
      for(let i=0;i<b.level;i++){
        ctx.fillStyle='#FFD700';ctx.beginPath();
        ctx.arc(pos.x-sz+i*6,pos.y-sz-4,2,0,Math.PI*2);ctx.fill();
      }
  ...
  for(let e of game.enemies){
    ...
    // HP bar
    let bw=e.radius*2,bh=4,bx=e.x-bw/2,by=e.y-e.radius-6;
  ...
  for(let [b,t] of game.laserTargets){
    ctx.strokeStyle=AGES[game.age].accent;ctx.lineWidth=3;ctx.globalAlpha=0.6+Math.random()*0.4;
...
function showInfoPopup(b,px,py){
  ...
  document.getElementById('info-name').textContent=def.icon+' '+def.name+' Lv'+(b.level+1);
  let stats='';
  if(b.cat==='tower'){
...
function updateBuildCards(){
  ...
  items.forEach((def,i)=>{
    ...
    card.innerHTML=`<div class="card-icon">${def.icon}</div><div class="card-name">${def.name}</div><div class="card-cost">💰 ${def.cost}</div>`;
    if(def.age<=game.age){
      card.onclick=(e)=>{
        e.stopPropagation();
